...
public class HistoricoLaboratorioService {
	...
	public List<HistoricoLaboratorioDTO> listarTodos(){
		return historicoLaboratorioRepository.findAll()
				.stream()
	...
	public List<HistoricoLaboratorioDTO> listarPorLaboratorio(Long laboratorioId){
		return historicoLaboratorioRepository.findByLaboratorioId(laboratorioId)
				.stream()
				.map(HistoricoLaboratorioDTO::new)
				.toList();
	}
	...
	public List<HistoricoLaboratorioDTO> listarPorProduto(Long produtoId){
		return historicoLaboratorioRepository.findByProdutoId(produtoId)
				.stream()
				.map(HistoricoLaboratorioDTO::new)
				.toList();
	}
	...
	public List<HistoricoLaboratorioDTO> listarPorPedido(Long pedidoId){
		return historicoLaboratorioRepository.findByPedidoId(pedidoId)
				.stream()
				.map(HistoricoLaboratorioDTO::new)
				.toList();
	}
	...
	@Transactional(readOnly = true)
	public List<HistoricoLaboratorioDTO> listarPorPeriodo(Long laboratorioId, LocalDate dataInicio, LocalDate dataFim){
		return historicoLaboratorioRepository.findByLaboratorioIdAndPeriodo(laboratorioId, dataInicio, dataFim)
				.stream()
				.map(HistoricoLaboratorioDTO::new)
				.toList();
	}
