...
public class ResiduoService {
	...
	public ResiduoResponseDTO criar(CriarResiduoRequestDTO dto) {
		...
		Residuo residuo = Residuo.builder().laboratorio(laboratorio).gerador(gerador).projeto(projeto)
				.descricao(dto.getDescricao()).processoOrigem(dto.getProcessoOrigem())
				.estadoFisico(dto.getEstadoFisico()).recipiente(dto.getRecipiente()).quantidade(dto.getQuantidade())
				.unidadeMedida(dto.getUnidadeMedida()).nivelRiscoInformado(dto.getNivelRiscoInformado())
	...
	private Projeto buscarProjeto(UUID projetoId, Laboratorio laboratorio) {
		...
		Projeto projeto = TenantContext.unidadeAtual().flatMap(
				unidadeId -> projetoRepository.findByPublicIdAndLaboratorioUnidadePublicId(projetoId, unidadeId))
				.orElseGet(() -> {
					if (TenantContext.ativo()) {
