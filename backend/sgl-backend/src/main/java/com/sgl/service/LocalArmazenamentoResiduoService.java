...
public class LocalArmazenamentoResiduoService {
	...
	public List<LocalArmazenamentoResiduoResponseDTO> listarTodos() {
		...
		List<LocalArmazenamentoResiduo> locais = TenantContext.unidadeAtual()
				.map(localArmazenamentoResiduoRepository::findByUnidadePublicIdOrderByNomeAsc)
				.orElseGet(localArmazenamentoResiduoRepository::findAll);

		return locais.stream().map(LocalArmazenamentoResiduoResponseDTO::new).toList();
	}
	...
	public List<LocalArmazenamentoResiduoResponseDTO> listarAtivos() {
		...
		List<LocalArmazenamentoResiduo> locais = TenantContext.unidadeAtual()
				.map(localArmazenamentoResiduoRepository::findByUnidadePublicIdAndAtivoTrueOrderByNomeAsc)
				.orElseGet(localArmazenamentoResiduoRepository::findByAtivoTrueOrderByNomeAsc);
	...
	private LocalArmazenamentoResiduo buscarLocalNoTenant(UUID id) {
		...
		return TenantContext.unidadeAtual().flatMap(
				unidadeId -> localArmazenamentoResiduoRepository.findByPublicIdAndUnidadePublicId(id, unidadeId))
				.orElseGet(() -> {

					if (TenantContext.ativo()) {

						throw new ResourceNotFoundException("Local de armazenamento de resíduo", id);
					}

					return localArmazenamentoResiduoRepository.findByPublicId(id)
